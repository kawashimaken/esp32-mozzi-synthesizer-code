...
// EventDelay instance definition (only one place)
...
int audio_update() {
  ...
  return ((code >> 2) + snare3.next() + snare4.next() + snap1.next() + 
          kick9.next() + (hihat1.next()) + (hihat2.next()) + 
          clap.next() + water1.next()) >> 2;
}
...
void melo_part() {
  if (seq_melo_trig[0][master_count]) {
    ...
    envB0.setADLevels((byte)180, (byte)100);
  ...
  if (seq_melo_trig[1][master_count]) {
    ...
    envB1.setADLevels((byte)180, (byte)100);
  ...
  if (seq_melo_trig[2][master_count]) {
    ...
    envB2.setADLevels((byte)180, (byte)100);
  ...
  if (seq_melo_trig[3][master_count]) {
    ...
    envB3.setADLevels((byte)180, (byte)100);
...
// Mozzi 2.0 compatible analog read function
int mozzi_analog_read(unsigned char pin) {
  return mozziAnalogRead<12>(pin);  // explicitly specify 12-bit resolution
}
