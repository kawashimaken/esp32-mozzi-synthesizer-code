...
// Remove direct EventDelay access, use wrapper functions
...
// Debounce variables for melody sequences 12
unsigned long last_melo12_button_time = 0;
bool prev_melo12_up = false;
bool prev_melo12_down = false;
bool prev_melo12_left = false;
bool prev_melo12_right = false;
bool prev_melo12_confirm = false;
bool prev_melo12_cancel = false;

void set_melo_seq_12() {
  unsigned long current_time = millis();
  
  // Check debounce time (for melody 12)
  if (current_time - last_melo12_button_time >= debounce_delay) {
    // Cursor movement (only when confirm button is not pressed)
    if (button_pressed(cross_button_up, prev_melo12_up) && !digitalRead(button_confirm)) {
      cursor_y_melo_12 = max(0, cursor_y_melo_12 - 1);
      last_melo12_button_time = current_time;
    }
    if (button_pressed(cross_button_down, prev_melo12_down) && !digitalRead(button_confirm)) {
      cursor_y_melo_12 = min(3, cursor_y_melo_12 + 1);
      last_melo12_button_time = current_time;
    }
    if (button_pressed(cross_button_right, prev_melo12_right) && !digitalRead(button_confirm)) {
      cursor_x_melo_12 = min(7, cursor_x_melo_12 + 1);
      last_melo12_button_time = current_time;
    }
    if (button_pressed(cross_button_left, prev_melo12_left) && !digitalRead(button_confirm)) {
      cursor_x_melo_12 = max(0, cursor_x_melo_12 - 1);
      last_melo12_button_time = current_time;
    }
  } else {
    // Update previous states even within debounce time
    prev_melo12_up = digitalRead(cross_button_up);
    prev_melo12_down = digitalRead(cross_button_down);
    prev_melo12_left = digitalRead(cross_button_left);
    prev_melo12_right = digitalRead(cross_button_right);
    prev_melo12_confirm = digitalRead(button_confirm);
    prev_melo12_cancel = digitalRead(button_cancel);
  }
